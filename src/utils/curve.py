...
def curve_interp(src, samples, index):
    ...
    for i in xrange(1, accu_length.shape[0]):
...
def curve_fitting(points, samples, index):
    ...
    if valid_points.shape[0] == 2:
        ...
        for i in xrange(functions.shape[0]):
        ...
        for i in xrange(valid_points.shape[0] - 4, -1, -1):
    ...
    for i in xrange(functions.shape[0]):
        ...
        for j in xrange(samples_per_segment):
            t = step * j
            rawcurve[i * samples_per_segment + j, :] = np.asarray([functions[i, 1] + functions[i, 2] * t + functions[i, 3] * t * t + functions[i, 4] * t * t * t,
...
def points_to_heatmap_rectangle_68pt(points,
                                     heatmap_num=13,
    ...
    for i in range(points.shape[0]):
        points[i][0] *= (float(heatmap_size[1]) / float(label_size[1]))
        points[i][1] *= (float(heatmap_size[0]) / float(label_size[0]))

    align_on_curve = [0] * heatmap_num
    curves = [0] * heatmap_num
    align_on_curve[0] = np.zeros((17, 2))
    align_on_curve[1] = np.zeros((5, 2))
    align_on_curve[2] = np.zeros((5, 2))
    align_on_curve[3] = np.zeros((4, 2))
    align_on_curve[4] = np.zeros((5, 2))
    align_on_curve[5] = np.zeros((4, 2))
    align_on_curve[6] = np.zeros((4, 2))
    align_on_curve[7] = np.zeros((4, 2))
    align_on_curve[8] = np.zeros((4, 2))
    align_on_curve[9] = np.zeros((7, 2))
    align_on_curve[10] = np.zeros((5, 2))
    align_on_curve[11] = np.zeros((5, 2))
    align_on_curve[12] = np.zeros((7, 2))
    ...
    align_on_curve[5][0] = points[36]
    align_on_curve[5][1] = points[37]
    align_on_curve[5][2] = points[38]
    align_on_curve[5][3] = points[39]

    align_on_curve[6][0] = points[39]
    align_on_curve[6][1] = points[40]
    align_on_curve[6][2] = points[41]
    align_on_curve[6][3] = points[36]
    ...
    align_on_curve[8][0] = points[45]
    align_on_curve[8][1] = points[46]
    align_on_curve[8][2] = points[47]
    align_on_curve[8][3] = points[42]
    ...
    heatmap = np.zeros((heatmap_size[0], heatmap_size[1], heatmap_num))
    for i in range(heatmap_num):
        curve_map = np.full((heatmap_size[0], heatmap_size[1]), 255, dtype=np.uint8)
        ...
        if len(valid_points) > 1:
            ...
            for j in range(curves[i].shape[0]):
                if (int(curves[i][j, 0] + 0.5) >= 0 and int(curves[i][j, 0] + 0.5) < heatmap_size[1] and
                    int(curves[i][j, 1] + 0.5) >= 0 and int(curves[i][j, 1] + 0.5) < heatmap_size[0]):
                    curve_map[int(curves[i][j, 1] + 0.5), int(curves[i][j, 0] + 0.5)] = 0

        image_dis = cv2.distanceTransform(
            curve_map, cv2.cv.CV_DIST_L2, cv2.cv.CV_DIST_MASK_PRECISE)
